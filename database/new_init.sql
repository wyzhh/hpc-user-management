...
-- 初始化默认管理员
INSERT INTO admins (username, full_name, email, password_hash, role, auth_type) VALUES
('admin', '系统管理员', 'admin@hpc.university.edu', '$2b$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'super_admin', 'local');

-- 创建兼容视图 (保持现有API兼容性)
CREATE VIEW pis_legacy_view AS
SELECT 
    p.id,
    COALESCE(u.ldap_dn, '') as ldap_dn,
    u.username,
    u.full_name,
    u.email,
    u.phone,
...
CREATE VIEW students_legacy_view AS  
SELECT
    s.id,
    u.username,
